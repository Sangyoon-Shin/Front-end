...
import styles from "./BoardCode.module.css";
...
const BoardCode = () => {
    ...
    const getBoard = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // codingCreatedTime 변환
          const formattedDate = new Date(data.codingCreatedTime).toLocaleString('ko-KR', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            hour: '2-digit',
          ...
          // 상태 업데이트
          setContent(data.codingContents);
          setTitle(data.codingTitle);
  ...
  useEffect(() => {
    const fetchHeartStatus = async () => {
      try {
        const response = await fetch(`${BASE_URL}/coding/${id}/like-status`, {
          method: 'GET',
...
useEffect(() => {
  const fetchComments = async () => {
    try {
      const response = await fetch(`${BASE_URL}/coding/${id}/comments`, {
        method: "GET",
        headers: getAuthHeaders(),
      });
...
const handleAddComment = async () => {
  if (commentContent.trim() !== '') {
    ...
    try {
      const response = await fetch(`${BASE_URL}/coding/${id}/comments/add?content=${encodeURIComponent(commentContent)}&anonymousId=${encodeURIComponent(anonymousId)}`, {
        method: 'POST',
        headers: getAuthHeaders(),
...
const handleAddReply = async (index) => {
  if (replyContents[index]?.trim() !== '') {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/coding/${id}/comments/add?parentCommentId=${parentCommentId}&content=${encodeURIComponent(content)}&anonymousId=${encodeURIComponent(anonymousId)}`,
        {
          method: 'POST',
          headers: getAuthHeaders(),
  ...
  const handleHeartClick = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/coding/${id}/like`, {
        method: 'POST',
        headers: getAuthHeaders(),
      ...
      console.log(`${BASE_URL}/coding/${id}/like`);
  ...
  const submitReport = async () => {
    try {
      ...
      const url = `${BASE_URL}/coding/${id}/report?reason=${encodeURIComponent(reason)}&reporterId=${encodeURIComponent(reporterId)}`;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.location.href = "/codingboardPage"; // 목록 페이지 경로로 이동
      } else {
        const errorData = await response.json();
        console.error("삭제 실패:", errorData);
        alert("게시글 삭제에 실패했습니다.");
        console.log(`${BASE_URL}/coding/delete/${id}`);
      }
    } catch (error) {
