...
import styles from "./BootBoard.module.css";
...
const BootBoard = () => {
  ...
  useEffect(() => {
    const fetchBoard = async () => {
      try {
        ...
        setTitle(data.bootcampTitle);
        setContent(data.bootcampContents);
      } catch (error) {
    ...
    const fetchComments = async () => {
      try {
        const response = await fetch(`${BASE_URL}/bootcamp/${id}/comments`, {
          method: 'GET',
          headers: getAuthHeaders(),
        });
        if (!response.ok) {
    ...
    const fetchHeartStatus = async () => {
      try {
        const response = await fetch(`${BASE_URL}/bootcamp/${id}/like-status`, {
          method: 'GET',
          headers: getAuthHeaders(),
        });
        if (response.ok) {
  ...
  // 게시글 내용 수정(부트캠프 게시판에서 수정 기능을 원한다면)
  ...
  const handleHeartClick = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/bootcamp/${id}/like`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const requestData = {
        ...
        targetType: 'bootcamp', // 부트캠프게시판
        targetId: id, // 게시글 ID
      };

      const response = await fetch(`${BASE_URL}/bootcamp/${id}/comments/add`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const submitReport = async () => {
    const boardType = 'Bootcamp'; // 고정값 설정
