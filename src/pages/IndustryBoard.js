...
import styles from "./IndustryBoard.module.css";
  ...
  useEffect(() => {
    ...
    useEffect(() => { // 댓글 불러오기
      const fetchComments = async () => {
        try {
          const response = await fetch(`${BASE_URL}/study/${id}/comments`, {
            method: 'GET',
            headers: getAuthHeaders(),
          });
      ...
      const getBoard = async () => { // 게시판 불러오기
        const accessToken = localStorage.getItem('accessToken');
        console.log(id);
        fetch(`http://info-rmation.kro.kr/api/board/studies/${id}`, {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
            'ngrok-skip-browser-warning': 1
          }
    ...
    const handleAddComment = async () => {
      if (commentContent.trim() !== '') {
        ...
        try {
          const response = await fetch(`${BASE_URL}/study/${id}/comments/add?content=${encodeURIComponent(commentContent)}&anonymousId=${encodeURIComponent(anonymousId)}`, {
            method: 'POST',
            headers: getAuthHeaders(),
    ...
    const submitReport = async () => {
      try {
        ...
        const url = `${BASE_URL}/study/${id}/report?reason=${encodeURIComponent(reason)}&reporterId=${encodeURIComponent(reporterId)}`;
  ...
  return (
    <div>
      ...
      <div className={styles["content"]}>
        ...
        />
        <h1 className={styles["title-text2"]}>산업연계 게시판</h1>
