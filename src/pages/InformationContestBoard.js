...
import styles from "./InformationContestBoard.module.css";
...
const InformationContestBoard = () => {
    ...
    const getBoard = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // competitionCreatedTime 변환
          const formattedDate = new Date(data.competitionCreatedTime).toLocaleString('ko-KR', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            hour: '2-digit',
          ...
          // 상태 업데이트
          setContent(data.competitionContents);
          setTitle(data.competitionTitle);
  ...
  useEffect(() => {
    const fetchHeartStatus = async () => {
      try {
        const response = await fetch(`${BASE_URL}/competition/${id}/like-status`, {
          method: 'GET',
...
useEffect(() => {
  const fetchComments = async () => {
    try {
      const response = await fetch(`${BASE_URL}/competition/${id}/comments`, {
        method: "GET",
        headers: getAuthHeaders(),
      });
...
const handleAddComment = async () => {
  if (commentContent.trim() !== '') {
    ...
    try {
      const response = await fetch(`${BASE_URL}/competition/${id}/comments/add?content=${encodeURIComponent(commentContent)}&anonymousId=${encodeURIComponent(anonymousId)}`, {
        method: 'POST',
        headers: getAuthHeaders(),
...
const handleAddReply = async (index) => {
  if (replyContents[index]?.trim() !== '') {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/competition/${id}/comments/add?parentCommentId=${parentCommentId}&content=${encodeURIComponent(content)}&anonymousId=${encodeURIComponent(anonymousId)}`,
        {
          method: 'POST',
          headers: getAuthHeaders(),
  ...
  const handleHeartClick = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/competition/${id}/like`, {
        method: 'POST',
        headers: getAuthHeaders(),
      ...
      console.log(`${BASE_URL}/competition/${id}/like`);
  ...
  const submitReport = async () => {
    try {
      ...
      const url = `${BASE_URL}/competition/${id}/report?reason=${encodeURIComponent(reason)}&reporterId=${encodeURIComponent(reporterId)}`;
  ...
  const handleDelete = async () => {
    ...
    try {
      const accessToken = localStorage.getItem('accessToken');
      const response = await fetch(`${BASE_URL}/competition/delete/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'ngrok-skip-browser-warning': 1
      ...
      if (response.ok) {
        ...
        console.log(`${BASE_URL}/competition/delete/${id}`);
      }
    } catch (error) {
...
const handleScrap = async () => {
  try {
    const response = await fetch(`${BASE_URL}/competition/${id}/scrap`, {
      method: 'POST',
...
const handleEdit = async () => {
  try {
    // API 요청 보내기
    const response = await fetch(`${BASE_URL}/competition/update/${id}`, {
      method: "GET",
      headers: getAuthHeaders(),
    });
...
export default InformationContestBoard;
